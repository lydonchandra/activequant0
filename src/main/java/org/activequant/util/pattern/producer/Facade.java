 ...
 *  @author Erik Nijkamp
 */
public class Facade<Integration> {
	
	protected Integration createIntegration(String classname,
			Properties config) throws Exception {
		return new Producer<Integration>().create(classname, config);
	}

	protected <T1> Integration createIntegration(String classname, T1 t1,
			Properties config) throws Exception {
		return new Producer<Integration>().create(classname, t1, config);
	}

	protected <T1, T2> Integration createIntegration(String classname, T1 t1,
			T2 t2, Properties config) throws Exception {
		return new Producer<Integration>().create(classname, t1, t2, config);
	}

	protected <T1, T2, T3> Integration createIntegration(String classname,
	...
	protected <T1, T2, T3, T4> Integration createIntegration(String classname,
