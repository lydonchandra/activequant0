...
public interface IOrderTracker {
	...
	/**
	 * Broker-assigned id for this order processing.
	 * 
	 * @return id.
	 */
	String getBrokerAssignedId();
	...
	/**
	 * Returns null if order is still open. Else returns order completion event.
	 * This is a convenience method. Alternative would be to attach to the
	 * {@link #getOrderEventSource() orderEventSource} and wait for completion
	 * event fired.
	 * 
	 * @return order completion event, or null if not yet completed.
	 */
	OrderCompletionEvent getOrderCompletion();
}
