 ...
 *  @author Erik Nijkamp
 */
public class SelectChain extends ChainBase {
	...
	public SelectChain(IFilter...transforms) {
		super(transforms);
	}
	
	public SelectChain(int selected, IFilter...transforms) {
		super(transforms);
		this.selected = selected;
	}
	
	public CandleSeries[] process(CandleSeries... sourceSeries) throws Exception {
		CandleSeries processed[] = super.process(new CandleSeries[]{sourceSeries[selected]});
		sourceSeries[selected] = processed[0];
		// return
		return sourceSeries;
	}
}