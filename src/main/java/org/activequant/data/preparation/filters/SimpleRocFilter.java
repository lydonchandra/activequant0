 ...
 *  @author Erik Nijkamp
 */
public class SimpleRocFilter extends FilterBase {
	
	public SimpleRocFilter() {
		super();
	}
	...
	public CandleSeries process(CandleSeries series) throws Exception {
		...
		newSeries.clear();
		// fill gap
		newSeries.add(0, new Candle(series.getInstrumentSpecification()));
		// calculate
	...
	private Candle roc(Candle previousCandle, Candle currentCandle) {
		...
		return new Candle(currentCandle.getInstrumentSpecification(), currentCandle.getTimeStamp(), rocDoubles);
	}
}