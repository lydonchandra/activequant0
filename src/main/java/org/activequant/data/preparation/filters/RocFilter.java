 ...
 *  @author Erik Nijkamp
 */
public class RocFilter extends FilterBase {
	...
	public RocFilter(int days) {
		super();
		this.days = days;
	}
	
	public RocFilter(int days, AdjustPolicy adjustPolicy) {
		super();
		this.days = days;
		...
		for(int i = 0; i < result.length; i++) {
			...
			for(int j = 0; j < series.size()-days; j++) {
				if(adjustPolicy == AdjustPolicy.STANDARD) {
					result[i][j+days] = FinancialLibrary.ROC(days, ohlc[i], j);
				} else {
					result[i][j+days] = FinancialLibrary.ROC(days, ohlc[i], j) / 100.0;
				}
			}
		}
		return result;		
