...
import org.activequant.data.retrieval.integration.opentick.OpentickTradeIndicationSeriesSource;
 ...
 */
public class OpentickTradeIndicationSeriesSourceTest {
	...
	public void testOpentickUsingMockFactory() throws Exception {		
		...
		OpentickTradeIndicationSeriesSource reader = new OpentickTradeIndicationSeriesSource();
		IConnectionFactory factory = new MockOpentickConnectionFactory();
		reader.setConnectionFactory(factory);
		
        TradeIndicationSeries series = reader.fetch(query);
        System.out.println("Series size: " + series.size());
        for(TradeIndication c : series.getTradeIndications()) {
